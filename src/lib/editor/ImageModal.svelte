<script lang="ts">
  ...
  import { onMount } from "svelte";
	...
	import { post } from "../../routes/editor/post.svelte";
  ...
  <div class="modal modal-open">
    <div class="modal-box max-w-2xl">
      ...
      <div class="tabs tabs-boxed mt-4">
        <button class={`tab ${activeTab === 'url' ? 'tab-active' : ''}`} onclick={() => activeTab = 'url'}>URL</button>
        <button class={`tab ${activeTab === 'upload' ? 'tab-active' : ''}`} onclick={() => activeTab = 'upload'}>Upload</button>
        <button class={`tab ${activeTab === 'library' ? 'tab-active' : ''}`} onclick={() => activeTab = 'library'}>Library</button>
      </div>
