...
import { convertValue, parseData } from "./utils";
...
export class Query<T = any> {
  ...
  private collectionId: Collection;
  ...
  async get<G = T>(): Promise<G> {
    ...
    const url =
      `https://firestore.googleapis.com/v1/projects/${this.projectId}/databases/(default)/documents:runQuery?key=${this.apiKey}`;
